    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud)
{
	...
	pcl::PointXYZ shoulder_midpoint(
	    (left_shoulder.x + right_shoulder.x) / 2,
	    (left_shoulder.y + right_shoulder.y) / 2,
	    (left_shoulder.z + right_shoulder.z) / 2);
	pcl::PointXYZ hip_midpoint(
	    (left_hip.x + right_hip.x) / 2,
	    (left_hip.y + right_hip.y) / 2,
	    (left_hip.z + right_hip.z) / 2);
	pcl::PointXYZ shoulder_hip_midpoint(
	...
	float avg_y = (left_shoulder.y + right_shoulder.y + left_hip.y + right_hip.y) / 4.0;
	float avg_x = (left_shoulder.x + right_shoulder.x + left_hip.x + right_hip.x) / 4.0;
	...
	pcl::PointXYZ min_z_point = *std::min_element(intersection_cloud->points.begin(), intersection_cloud->points.end(), [](const auto& a, const auto& b) {
		return a.z < b.z;
	});
